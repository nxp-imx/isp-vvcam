...
static struct v4l2_device * basler_v4l2_dev = 0;
			      ...
			      struct register_access *ra_p)
{
	...
	if (ra_p->data_size > sizeof(ra_p->data)){
		dev_err(&client->dev, "i2c burst array too big, max allowed %ld, got %d\n", sizeof(ra_p->data), ra_p->data_size);
		return -EINVAL;
	}
...
int basler_ioc_qcap(void *dev, void *args)
{
        ...
        case BASLER_IOC_READ_REGISTER:
          {
            ...
            ret = basler_read_register_chunk(client, ra_p->data, ra_p->data_size, ra_p->address);
          }
          break;
			       ...
			       const struct i2c_device_id *id)
{
	...
	if (ret)
        ...
        if (basler_v4l2_dev == 0) {
                dev_err(dev, "%s video device not supplied\n", SENSOR_NAME);
		goto handler_cleanup;
        }

        ret = v4l2_device_register_subdev(basler_v4l2_dev, &sensor->sd);
	if (ret)
		goto handler_cleanup;
...
{
  basler_v4l2_dev = vdev;
  return i2c_add_driver(&basler_camera_i2c_driver);
}

void basler_hw_unregister(void)
{
  i2c_del_driver(&basler_camera_i2c_driver);
}
