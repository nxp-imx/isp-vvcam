...
static struct ov2775_mode_info ov2775_mode_info_data[2][ov2775_mode_MAX + 1] = {
	...
	},
	{
		...
		ov2775_init_setting_1080p,//ov2775_setting_30fps_1080P_1920_1080,
		ARRAY_SIZE(ov2775_init_setting_1080p)}, //ov2775_init_setting_1080p
...
static enum ov2775_frame_rate to_ov2775_frame_rate(struct v4l2_fract *timeperframe)
{
	...
	for (i = 0; i < ARRAY_SIZE(ov2775_framerates); i++) {
		for (j = 0; j < (ov2775_mode_MAX + 1); j++) {
			if (fie->width == ov2775_mode_info_data[i][j].width
			 ...
			 && ov2775_mode_info_data[i][j].init_data_ptr != NULL) {
				count++;
			}
			if (fie->index == (count - 1)) {
				fie->interval.denominator = ov2775_framerates[i];
				return 0;
			}
		}
	}
			...
			const struct i2c_device_id *id)
{
	...
	if (!gpio_is_valid(sensor->pwn_gpio))
		...
		retval = devm_gpio_request_one(dev, sensor->pwn_gpio,
				GPIOF_OUT_INIT_HIGH, "ov2775_mipi_pwdn");
		if (retval < 0) {
	...
	if (!gpio_is_valid(sensor->rst_gpio))
		...
		retval = devm_gpio_request_one(dev, sensor->rst_gpio,
				GPIOF_OUT_INIT_HIGH, "ov2775_mipi_reset");
		if (retval < 0) {
	...
	retval = ov2775_read_reg(sensor, OV2775_CHIP_ID_LOW_BYTE,
				&chip_id_low);
